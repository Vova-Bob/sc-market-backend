...
async function testThreadCreation() {
  ...
  console.log(
    `   DISCORD_QUEUE_URL: ${env.DISCORD_QUEUE_URL ? "✅ Set" : "❌ Missing"}`,
  )
  console.log(
    `   BACKEND_QUEUE_URL: ${env.BACKEND_QUEUE_URL ? "✅ Set" : "❌ Missing"}`,
  )
  console.log("")
  ...
  try {
    ...
    console.log("✅ Result:")
    console.log(
      `   Status: ${result1.result.failed ? "❌ Failed" : "✅ Success"}`,
    )
  ...
  try {
    ...
    console.log("✅ Result:")
    console.log(
      `   Status: ${result2.result.failed ? "❌ Failed" : "✅ Success"}`,
    )
    console.log(`   Message: ${result2.result.message}`)

    if (result2.result.failed) {
      console.log(
        "ℹ️  This is expected since we don't have real Discord configuration in database",
      )
    } else {
