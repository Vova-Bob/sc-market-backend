...
export async function has_permission(
  ...
  const user_roles = await database.getMemberRoles(contractor_id, user_id)
...
export async function outranks(
  ...
  // let roles = await database.getContractorRoles({contractor_id})
  const lower_roles = await database.getMemberRoles(contractor_id, lower_id)
  const higher_roles = await database.getMemberRoles(contractor_id, higher_id)

  const low_min = Math.min(...lower_roles.map((r) => r.position))
  const high_min = Math.min(...higher_roles.map((r) => r.position))
...
export async function can_manage_role(
  ...
  // let roles = await database.getContractorRoles({contractor_id})
  const user_roles = await database.getMemberRoles(contractor_id, user_id)
  const role = await database.getContractorRole({ contractor_id, role_id })

  const user_min = Math.min(...user_roles.map((r) => r.position))
