...
orderSettingsRouter.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json(createErrorResponse({ error: "Failed to fetch order settings" }))
    }
  }
)
...
orderSettingsRouter.post(
  ...
  async (req, res) => {
    const user = req.user as User
    const { setting_type, message_content, enabled = true }: CreateOrderSettingRequest = req.body

    if (!setting_type || !message_content) {
      res.status(400).json(createErrorResponse({ error: "setting_type and message_content are required" }))
      return
    }
    ...
    try {
      ...
      const existing = await database.getOrderSetting('user', user.user_id, setting_type)
      if (existing) {
        ...
        res.status(200).json(createResponse({ setting: serializeOrderSetting(updated) }))
        return
      }
      ...
      res.status(201).json(createResponse({ setting: serializeOrderSetting(setting) }))
    } catch (error) {
      ...
      res.status(500).json(createErrorResponse({ error: "Failed to create order setting" }))
    }
  }
)
...
orderSettingsRouter.put(
  ...
  async (req, res) => {
    ...
    if (message_content === undefined && enabled === undefined) {
      res.status(400).json(createErrorResponse({ error: "At least one field must be provided for update" }))
      return
    }

    try {
      // First check if the setting exists and belongs to the user
      const existing = await database.knex<DBOrderSetting>('order_settings')
        .where({ id, entity_type: 'user', entity_id: user.user_id })
        .first()

      if (!existing) {
        res.status(404).json(createErrorResponse({ error: "Order setting not found" }))
        return
      }

      const updated = await database.updateOrderSetting(id, { message_content, enabled })
      res.json(createResponse({ setting: serializeOrderSetting(updated) }))
    } catch (error) {
      console.error('Error updating order setting:', error)
      res.status(500).json(createErrorResponse({ error: "Failed to update order setting" }))
    }
  }
)
...
orderSettingsRouter.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json(createErrorResponse({ error: "Failed to delete order setting" }))
    }
  }
)
...
orderSettingsRouter.get(
  ...
  async (req, res) => {
    ...
    try {
      const settings = await database.getOrderSettings('contractor', contractor.contractor_id)
      ...
      res.status(500).json(createErrorResponse({ error: "Failed to fetch contractor order settings" }))
    }
  }
)
...
orderSettingsRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      // Check if setting already exists
      const existing = await database.getOrderSetting('contractor', contractor.contractor_id, setting_type)
      if (existing) {
        // Update existing setting instead of creating new one
      ...
      res.status(500).json(createErrorResponse({ error: "Failed to create contractor order setting" }))
    }
  }
)
...
orderSettingsRouter.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const existing = await database.knex<DBOrderSetting>('order_settings')
        .where({ id, entity_type: 'contractor', entity_id: contractor.contractor_id })
        .first()
      ...
      res.status(500).json(createErrorResponse({ error: "Failed to update contractor order setting" }))
    }
  }
)
...
orderSettingsRouter.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json(createErrorResponse({ error: "Failed to delete contractor order setting" }))
    }
  }
)
