...
ordersRouter.get(
  ...
  async (req, res, next) => {
    ...
    if (!contractor) {
      res.status(400).json({ message: req.t("orders.invalidContractor") })
      return
    }
...
ordersRouter.get(
  ...
  async (req, res, next) => {
    ...
    if (!contractor) {
      res.status(404).json({ message: req.t("orders.invalidContractor") })
      return
    }
    ...
    if (
      ...
      res
        .status(403)
        .json({ message: req.t("errors.missingPermissions") })
      return
    }
...
ordersRouter.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      res.status(404).json({ message: req.t("orders.invalidOrder") })
      return
    }
    const user = req.user as User
    ...
    if (!["customer", "contractor"].includes(role)) {
      res.status(400).json({ message: req.t("orders.invalidRole") })
      return
    }
    ...
    if (!content) {
      res.status(400).json({ message: req.t("orders.emptyMessage") })
      return
    }

    if (!rating || rating > 5 || rating <= 0 || rating % 0.5 !== 0) {
      res.status(400).json({ message: req.t("orders.invalidRating") })
      return
    }
    ...
    if (existing) {
      res
        .status(409)
        .json({ message: req.t("orders.reviewExists") })
      return
    }
...
ordersRouter.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      res
        .status(400)
        .json(createErrorResponse({ message: req.t("orders.invalidOrder") }))
      return
    }
    const user = req.user as User
    ...
    if (order.assigned_id || order.contractor_id) {
      res
        .status(409)
        .json(
          createErrorResponse({ message: req.t("orders.alreadyAssigned") }),
        )
      return
    }
    ...
    if (!contractor) {
      ...
      if (apps.length > 0) {
        res.status(409).json(
          createErrorResponse({ message: req.t("orders.alreadyApplied") }),
        )
        return
      }
  ...
  async (req, res) => {
    if (req.order!.thread_id) {
      res
        .status(409)
        .json(
          createErrorResponse({ message: req.t("orders.threadExists") }),
        )
      return
    }
