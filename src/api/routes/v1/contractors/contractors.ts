  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (cobj) {
        res.status(409).json(
          createErrorResponse({
            message: req.t("errors.orgAlreadyRegistered"),
    ...
    if (await authorizeContractor(spectrum_id, user.user_id)) {
      ...
      res.status(403).json(
        createErrorResponse({
          message: req.t("errors.failedToAuthenticate"),
          status: req.t("common.error"),
        }),
      )
    }
...
contractorsRouter.get(
  ...
  async (req, res, next) => {
    ...
    if (!invite) {
      res
        .status(404)
        .json(createErrorResponse({ message: req.t("errors.invalidInvite") }))
      return
    }
...
contractorsRouter.post(
  ...
  async (req, res, next) => {
    ...
    if (role) {
      res
        .status(409)
        .json(createErrorResponse({ message: req.t("errors.alreadyMember") }))
      return
    }
...
contractorsRouter.get(
  ...
  async (req, res) => {
    ...
    if (unrelated) {
      res.status(403).json(
        createErrorResponse({
          message: req.t("errors.notAuthorizedViewData"),
        }),
      )
      return
    }
  ...
  async (req, res, next) => {
    ...
    if (!role) {
      res
        .status(404)
        .json(
          createErrorResponse({ message: req.t("errors.invalidRolePeriod") }),
        )
      return
    }
    ...
    if (
      ...
      res
        .status(403)
        .json(
          createErrorResponse({ message: req.t("errors.noPermissionsPeriod") }),
        )
      return
    }
    if (
  ...
  async (req, res, next) => {
    ...
    if (role_id === contractor.default_role) {
      res
        .status(403)
        .json(
          createErrorResponse({ message: req.t("errors.roleCannotBeRemoved") }),
        )
      return
    }
    ...
    res.status(204).json(createResponse({ result: req.t("common.success") }))
  },
)
...
contractorsRouter.post(
  ...
  async (req, res, next) => {
    ...
    if (!role) {
      res
        .status(404)
        .json(createErrorResponse({ message: req.t("errors.invalidRole") }))
      return
    }
  ...
  async (req, res, next) => {
    ...
    if (!target_is_member) {
      res
        .status(400)
        .json(
          createErrorResponse({ message: req.t("errors.invalidUserPeriod") }),
        )
      return
    }
    ...
    res.json(createErrorResponse({ result: req.t("common.success") }))
  },
)
  ...
  async (req, res, next) => {
    try {
      ...
      res
        .status(500)
        .json(
          createErrorResponse({ message: req.t("errors.internalServerError") }),
        )
      return
    }
  },
...
contractorsRouter.put(
  ...
  async (req, res, next) => {
    ...
    if (avatar_url && !avatar_url.match(external_resource_regex)) {
      res
        .status(400)
        .json(createErrorResponse({ message: req.t("errors.invalidUrl") }))
      return
    }
  ...
  async (req, res, next) => {
    ...
    if (!webhook_url || !name) {
      res
        .status(400)
        .json(
          createErrorResponse({ message: req.t("errors.invalidArguments") }),
        )
      return
    }
    ...
    try {
      ...
      res
        .status(400)
        .json(createErrorResponse({ message: req.t("errors.invalidActions") }))
      return
    }
    ...
    for (const username of usernames) {
      try {
        ...
        res
          ...
          .json(
            createErrorResponse({
              message: req.t("errors.invalidUserExclamation"),
            }),
          )
        return
      }
      ...
      if (invite_id) {
        ...
        if (!invites.length) {
          res
            .status(400)
            .json(
              createErrorResponse({ message: req.t("errors.invalidInvite") }),
            )
          return
        }
        ...
        if (role) {
          res
            .status(400)
            .json(
              createErrorResponse({ message: req.t("errors.alreadyMember") }),
            )
          return
        }
        ...
        if (!invites.length) {
          res
            ...
            .json(
              createErrorResponse({
                message: req.t("errors.invalidContractor"),
              }),
            )
          return
        }
...
contractorsRouter.post(
  ...
  async (req, res, next) => {
    ...
    if (roles.length) {
      res.status(400).json(
        createErrorResponse({
          message: req.t("errors.cannotLeaveOwnedContractor"),
        }),
      )
      return
    }
